...
from TGRobot import BOT_ID, pbot as app
from TGRobot import DRAGONS as SUDOERS
...
@app.on_message(
    filters.command("safepromote") & ~filters.edited & ~filters.private
)
@adminsOnly("can_promote_members")
async def promoteFunc(_, message: Message):
    user_id = await extract_user(message)
    ...
    await message.reply_text("Promoted with safe rights!")
