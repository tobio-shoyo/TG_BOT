...
def extract_user_and_text(
    ...
    if entities and ent and ent.offset == len(message.text) - len(text_to_parse):
        ...
        if not user_id:
            message.reply_text(
                "We have no records of this hunter or human in our Shadows Database"
                "you should reply to that person's message instead, or forward one of that user's messages."
            )
            return None, None
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found"):
            message.reply_text(
                "We have no records of this hunter or human in our Shadows Database - please forward a message from "
                "them to give me control! (like a shadow trace, I need a piece of them to be able "
...
def extract_unt_fedban(
    ...
    try:
        ...
        if excp.message in ("User_id_invalid", "Chat not found") and not isinstance(
            ...
            message.reply_text(
                "We have no records of this hunter or human in our Shadows Database "
                 "forward a message from them to give me control! (like a shadow trace, I need a piece of them to be able "
                "to execute certain commands...)"
            )
            return None, None
