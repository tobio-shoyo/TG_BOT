    ...
    def is_dev_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user.id in DEV_USERS:
            ...
            update.effective_message.reply_text(
                "This is a monarch restricted command."
...
def sudo_plus(func):
    ...
    def is_sudo_plus_func(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user and is_sudo_plus(chat, user.id):
            ...
            update.effective_message.reply_text(
                "Who is this unworthy human? ? You want a punch?"
            )
...
def whitelist_plus(func):
    ...
    def is_whitelist_plus_func(
        ...
        if user and is_whitelist_plus(chat, user.id):
            ...
            update.effective_message.reply_text(
                f"Maybe you aren't Shadow of my lord." #\nVisit @{SUPPORT_CHAT}"
            )
...
def user_admin(func):
    ...
    def is_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if user and is_user_admin(chat, user.id):
            ...
            update.effective_message.reply_text(
                "Who is this unworthy human? Why are you trying to order me?"
            )
...
def bot_admin(func):
    ...
    def is_admin(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            not_admin = "I'm not admin! - So I cant do the assigned task"
        else:
...
def bot_can_delete(func):
    ...
    def delete_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_delete = "(TwT) I can't delete messages here!\nEither I aint admin or You haven't given me enough permissions."
        else:
            cant_delete = f"(TwT) I can't delete messages in <b>{update_chat_title}</b>!\nMake sure I'm admin with full rights"
...
def can_pin(func):
    ...
    def pin_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_pin = "(TwT) I can't pin messages here!\nUwU You did'nt give me enough rights. Check if I can pin messages."
        else:
            cant_pin = f"(TwT) I can't pin messages in <b>{update_chat_title}</b>!\nMake sure I am admin with enough permissions to do the required task"
...
def can_promote(func):
    ...
    def promote_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_promote = "(TwT) I can't promote/demote people here!\nUwU You did'nt give me enough rights. Check if I can appoint new admins."
        else:
            cant_promote = (
                f"(TwT) I can't promote/demote people in <b>{update_chat_title}</b>!\n"
                f"OwO looks like (TwT) I can't do that Check if I am admin there and can appoint new admins."
            )
...
def can_restrict(func):
    ...
    def restrict_rights(update: Update, context: CallbackContext, *args, **kwargs):
        ...
        if update_chat_title == message_chat_title:
            cant_restrict = "(TwT) I can't restrict people here!\nUwU You did'nt give me enough rights. Check if I can restrict users."
        else:
            cant_restrict = f"(TwT) I can't restrict people in <b>{update_chat_title}</b>!\nOwO looks like I can't do that Check if I am admin there and can restrict users."
...
from TGRobot.modules import connection
