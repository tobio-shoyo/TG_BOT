...
import TGRobot.modules.sql.global_bans_sql as sql
from TGRobot.modules.sql.users_sql import get_user_com_chats
from TGRobot import (
    DEV_USERS,
    EVENT_LOGS,
    OWNER_ID,
...
from TGRobot.modules.helper_funcs.misc import send_to_list
...
def gban(update: Update, context: CallbackContext):
    ...
    if user_id == OWNER_ID:
        message.reply_text(
            "This user is The Shadow Monarch\nMy hubby... btw Why are you ridiculing yourself?"
        )
        return
    
    if int(user_id) in DEV_USERS:
        message.reply_text(
            "Mighty Divine Dragons are the divine Shadows I can't fight"
        )
        return

    if int(user_id) in DRAGONS:
        message.reply_text(
            "That's a Grand Marshal... He is the highest Shadow assigned by the Divine Dragons\n Only they can control them"
        )
        return

    if int(user_id) in DEMONS:
        message.reply_text(
            "Dude its a Marshal... Only Divine Dragons our my Hybby can subdue it..."
        )
        return

    if int(user_id) in TIGERS:
        message.reply_text("*Googles* How to kill a Shadow Commander?")
        return

    if int(user_id) in WOLVES:
        message.reply_text("Duh!! on drugs??? He is a Elite Knight")
        return

    if user_id == bot.id:
        message.reply_text("Why are you being dumb? There is no way I am going to do that.")
        return

    if user_id in [777000, 1087968824]:
        message.reply_text("Is that user anon?")
        return
...
def gbanstat(update: Update, context: CallbackContext):
    ...
    if len(args) > 0:
        if args[0].lower() in ["on", "yes"]:
            ...
            update.effective_message.reply_text(
                "Antispan is now disabled ❌ " "Your group wont wont be safe froms scammers, threats, raiders, spammers etc. anymore"
            )
    else:
        update.effective_message.reply_text(
