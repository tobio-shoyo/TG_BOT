...
import html
import TGRobot.modules.sql.blacklistusers_sql as sql
from TGRobot import (
    DEV_USERS,
    OWNER_ID,
    SUNG_ID,
...
from TGRobot.modules.helper_funcs.chat_status import dev_plus
...
BLACKLISTWHITELIST = [OWNER_ID] + DEV_USERS + DRAGONS + WOLVES + DEMONS + [SUNG_ID]
BLABLEUSERS = [OWNER_ID] + DEV_USERS + [SUNG_ID]
...
def bl_user(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("Stop playing with the blade or you might cut your fingers kid...")
        return ""

    if user_id in BLACKLISTWHITELIST:
        message.reply_text("No!\nNoticing Shadows is my job.")
        return ""
    ...
    try:
        ...
        if excp.message == "User not found":
            message.reply_text("I can't seem to find this civilian.")
            return ""
        else:
            raise
...
def unbl_user(update: Update, context: CallbackContext) -> str:
    ...
    if user_id == bot.id:
        message.reply_text("Don't joke around you know how the commands work? \n Maybye you are dumb")
        return ""
