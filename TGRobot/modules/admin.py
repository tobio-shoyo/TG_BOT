...
from TGRobot import DRAGONS, dispatcher
...
from TGRobot.helper_extra.admin_rights import (
    user_can_pin,
    user_can_promote,
    user_can_changeinfo,
)

from TGRobot.modules.helper_funcs.extraction import (
    extract_user,
    extract_user_and_text,
)
from TGRobot.modules.log_channel import loggable
from TGRobot.modules.helper_funcs.alternate import send_message
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    if (
        ...
        message.reply_text("You are a lower rank hunter to perform this task!")
        return
    ...
    if user_member.status == "administrator" or user_member.status == "creator":
        message.reply_text("How am I meant to assign shadow to someone that's already a ?")
        return
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            message.reply_text("I can't assign shadow to someone who isn't in the group.")
        else:
    ...
    bot.sendMessage(
        chat.id,
        f"Under assigned Shadow Protection <b>{user_member.user.first_name or user_id}</b> is a Hunter now!",
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    if user_member.status == "creator":
        message.reply_text("This person is Creator of this dungeon, how would I demote them?")
        return

    if not user_member.status == "administrator":
        message.reply_text("Can't assign shadow to civilians!")
        return

    if user_id == bot.id:
        message.reply_text("Y you bully me... pls dont demote me.")
        return
    ...
    try:
        ...
        bot.setChatAdministratorCustomTitle(chat.id, user_id, "Hunter")

        bot.sendMessage(
            chat.id,
            f"<b>{user_member.user.first_name or user_id}</b> demoted to Civilian!",
            parse_mode=ParseMode.HTML,
        )
...
def set_title(update: Update, context: CallbackContext):
    ...
    if user_member.status != "administrator":
        message.reply_text(
            "Can't set title for non-hunters!\nAssign Shadow to them to give them a name!"
        )
        return
    ...
    try:
        ...
        message.reply_text("UwU Hunter was promoted by someone else so I can't name this fellow!")
        return

    bot.sendMessage(
        chat.id,
        f"Assigned Shadow title for <code>{user_member.user.first_name or user_id}</code> "
